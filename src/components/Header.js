...
const Header = () => {
  ...
  return (
    <div className="flex justify-between items-center p-5 shadow-lg fixed top-0 left-0 right-0 mb-4 bg-white">
      <div className="">
        <Link to="/">
          <img className="h-8 mx-3 hover:opacity-75 transition-opacity duration-300" alt="logo" src={homeIcon} />
        </Link>
      </div>
      ...
      <div className="flex flex-row gap-12">
        <Link to={"/cart"}>
          <div className="relative text-2xl font-bold">
            <img className="h-8 mx-3  hover:opacity-75 transition-opacity duration-300" alt="icon" src={icon} />
            {!!totalQuanity && (
              <div
        ...
        <div className="">
          <img className="h-8 mx-3" alt="user" src={userIcon} />
        </div>
      </div>
    </div>
  );
